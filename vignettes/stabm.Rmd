...
In this example, we will analyse the stability of the feature selection of regression trees
...
```{r}
fit_tree = function(ratio = 0.67) {
    n = nrow(BostonHousing2)
    i = sample(n, n * ratio)
    model = rpart::rpart(medv ~ ., data = BostonHousing2, subset = i)
...
A quick analysis of the list reveals that nearly all features have been selected at least once:

```{r}
# Not selected at all:
setdiff(names(BostonHousing2), c("medv", unlist(selected_features)))
...
To finally quantify the selection frequencies, e.g. to compare the stability of regression trees to the stability of a different modeling approach, one of the implemented stability measures can be calculated:
```{r}
