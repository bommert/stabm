...
# remove variable that is a version of the target variable
...
fit_tree = function(ratio = 0.67) {
    n = nrow(dataset)
    i = sample(n, n * ratio)
    model = rpart::rpart(medv ~ ., data = dataset, subset = i, 
...
A quick analysis of the list reveals that three variables are selected in all repetitions while six other variables are only selected in some of the repetitions:
```{r}
